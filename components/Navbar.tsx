    ...
    <nav className="fixed top-0 w-full z-50 glass border-b border-zinc-800/50">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-8 text-sm font-medium">
          {navItems.map((item) => (
            <button
              ...
              className={`${
                currentPage === item ? 'text-white' : 'text-zinc-400 hover:text-white'
              } transition-colors`}
            >
              {item}
            </button>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#5865F2]/10 border border-[#5865F2]/30 text-[#5865F2] text-sm font-bold">
